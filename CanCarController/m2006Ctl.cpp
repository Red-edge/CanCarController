...
void m2006Ctl::m2006Init(struct can_frame *m2006rx) // 用于重定位指针
{
    ...
    m2006balpid.init_pid(1.0f, 0.03f, 0.0f, 2000.0f, 200.0f, 0.0f); // 平衡环
...
void m2006Ctl::m2006Update()
{
    ...
    for (int j = 0; j < 4; j++)
    {
        ...
        _tgtcur[j] = m2006balpid.pidUpdate(ax/10 - _tgtcur[j], j); // 这里应该是用速度环输出的电流（加速度）与角度（重力分量导致的加速度）做差得到一个目标角度
