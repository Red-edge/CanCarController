...
int main()//这个是用来参考的can接发函数
{
    ...
    frame.can_id = 0x200; // 设置 CAN ID
    frame.can_dlc = 8;    // 设置数据长度为 8 字节

    // 设置数据
    char ctrl[8] = {0x2F, 0x01, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00};
    char GetTemperature[8] = {0x02, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00};
    char GetTemperatureAck[4] = {0x4F, 0x00, 0x20, 0x00};
    std::memcpy(frame.data, ctrl, 8);
    ...
    while (true)
    {
        ...
        write(socketDescriptor, &frame, sizeof(frame));
        if (!strncmp((const char *)receivedFrame.data, (const char *)GetTemperatureAck, 4))
        {
            float a = (float)receivedFrame.data[4];
            printf("temprature = %f\n", a);
        }
        if (int16_t(GetTemperature[1]) >= 0x30)
        {
            GetTemperature[1] = char(int(GetTemperature[1]) - 1);
        }
        else if (int16_t(GetTemperature[1]) <= 0x10)
        {
            GetTemperature[1] = char(int(GetTemperature[1]) + 1);
        }
        printf("\n");
